...
Hourly precipitation, wind speed, wind direction, temperature, and relative humidity forecast for Brynsiencyn, Ynys Môn. Built as a single HTML file with no dependencies or build step.
...
https://api.open-meteo.com/v1/forecast
  ?latitude=53.1667
  &longitude=-4.3167
  &hourly=precipitation,windspeed_10m,winddirection_10m,temperature_2m,relativehumidity_2m
...
| `hourly=relativehumidity_2m` | Relative humidity at 2m height as a percentage |
...
The grid layout uses CSS Grid with eight columns: `hour | rain-bar | rain-value | wind-bar | wind-value | direction | temperature | humidity`.
...
| Function | What it does |
|---|---|
| `groupByDay(times, rain, wind, winddir, temp, humidity)` | Takes the flat hourly arrays from the API and groups them into an array of day objects, each containing a `date` and an `hours` array. Keys by local date parts (not UTC) to avoid BST midnight creating a phantom day with a single hour of data |
...
`fetchForecast()` is called on page load. It fetches the API, passes the five hourly arrays into `groupByDay()`, selects today's index, then calls `renderDayNav()` and `renderGrid()` to populate the page.
...
### Relative Humidity
Humidity is displayed as a plain percentage in muted blue. No colour thresholds are applied.
